...
> **本文件是 Reckoner 项目的唯一设计事实源（Single Source of Truth）。**
...
**电梯陈述**：Reckoner 是一个跑在 Claude Code / Cowork / Codex 上的 PM skill 插件包 + 决策内核。和普通 PM 助手最大的不同：每个 skill 跑完都把结论**回写内核**（假设台账 + 论点），跨会话记住你的假设、证据与到期项。它不是无状态的文档生成器，是一个会积累的**决策 OS**。

- **项目名 / 仓库名**：仓库名 `reckoner`（GitHub: <https://github.com/LuckyOneTwoThree/reckoner>）；产品名 **Reckoner**（中英文统一，无中文别名；旧中文别名「诤」已废弃）；「会顶嘴、有记忆的决策内核」定为产品口号。**照妖镜** 仅作 `assumption-xray` 这一个 skill 的昵称，不与其他层级并列。
...
- [x] 统一项目命名为 **Reckoner**：README / AGENTS / marketplace.json 全部对齐（口号定为「会顶嘴、有记忆的决策内核」；中文别名「诤」已废弃）。
...
- [ ] **第 0 位：Reckoner 自身 A/B 验证**——找 3 个 PM 真实跑一遍，量是否回来第二次。这是项目自身的承重假设，比引擎打磨更优先。
...
| decision-log schema | 已有 ≥5 条 validated/refuted（注：/retro + /lookup 已下移 P0，仅 decision-log schema 推迟） |
...
| Master Plan v1.1 | 2026-07-23 | 对齐 GitHub P0 现状：统一项目名 Reckoner、代码事实源改为 GitHub 仓库、细化 P0 收尾路线为 A/B/C（命名同步 / PROJECT-PLAN 回流 / 端到端实测 / 工程化完善）。 |
